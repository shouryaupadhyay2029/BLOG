...
export function SatyaMithyaPage() {
  ...
  const otherCases = [
    { number: '#002', title: 'Did Draupadī laugh at Duryodhana?', id: 'did-draupadi-really-laugh-at-duryodhana' },
    { number: '#003', title: 'Was Karṇa the greatest victim?', id: 'was-karna-the-greatest-victim' },
    { number: '#004', title: 'Are there really 33 crore gods?', id: 'are-there-really-33-crore-gods' },
  ];
  ...
  const relatedCases = [
    ...
    {
      id: 'does-hinduism-teach-everything-symbolic',
      number: 'CASE FILE 005',
      title: 'Does Hinduism Teach That Everything Is Symbolic?',
      status: 'Coming Soon',
      desc: 'Where do the scriptures describe literal reality, symbolic teaching, or both?'
    }
  ];
  ...
  const knowledgeNetwork = [
    ...
    { id: '002', title: 'Karma Means Fate?', status: 'faded', desc: 'Case File 002' },
    { id: '003', title: 'Ṛgveda and Creator Deity', status: 'faded', desc: 'Case File 003' },
    { id: '004', title: 'Itihāsa vs Mythology', status: 'faded', desc: 'Case File 004' },
        ...
        <section className="py-16 border-t border-[#C58B52]/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-8">
            {relatedCases.map((c, idx) => (
              <div
                key={idx}
                className={`border p-6 flex flex-col justify-between min-h-[160px] ${
                  c.status === 'Available'
                    ? 'border-[#9E2A2B]/40 bg-white/30 cursor-pointer hover:border-[#9E2A2B] shadow-sm'
                    : 'border-[#1C1C1A]/10 bg-[#1c1c1a]/5 opacity-60 cursor-not-allowed'
                }`}
              >
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`font-general text-[7px] uppercase tracking-widest px-2 py-0.5 border ${
                      c.status === 'Available' ? 'text-green-600 border-green-600 bg-green-50' : 'text-[#1C1C1A]/40 border-[#1C1C1A]/20'
                    }`}>
                      {c.status}
                    </span>
                  </div>
